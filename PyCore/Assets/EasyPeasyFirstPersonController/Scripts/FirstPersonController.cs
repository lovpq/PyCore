namespace EasyPeasyFirstPersonController
{
    ...
    public partial class FirstPersonController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // НЕ блокируем курсор сразу - это сделает NeedsManager или игрок через Escape
            ...
            // Кэшируем ссылку на SimplePythonUI
            pythonUI = FindFirstObjectByType<SimplePythonUI>();
        }

        private void Update()
        {
            // Обработка Escape для управления курсором (только если НЕТ открытых панелей)
            var keyboard = UnityEngine.InputSystem.Keyboard.current;
            bool anyPanelOpen = (Core.NeedsManager.Instance != null && Core.NeedsManager.Instance.IsAnyPanelOpen()) ||
                               (pythonUI != null && pythonUI.IsPanelOpen);
            
            if (keyboard != null && keyboard.escapeKey.wasPressedThisFrame)
            {
                if (anyPanelOpen)
                {
                    // Если панели открыты - закрываем их через их собственную логику (SimplePythonUI уже это делает)
                    Debug.Log("FirstPersonController: Escape pressed but panels are open, skipping cursor toggle");
                }
                else
                {
                    // Переключаем состояние курсора только если панелей нет
                    if (Cursor.lockState == CursorLockMode.Locked)
                    {
                        Cursor.lockState = CursorLockMode.None;
                        Cursor.visible = true;
                        Debug.Log("FirstPersonController: Cursor unlocked by Escape");
                    }
                    else
                    {
                        Cursor.lockState = CursorLockMode.Locked;
                        Cursor.visible = false;
                        Debug.Log("FirstPersonController: Cursor locked by Escape");
                    }
                }
            }
            
            // Проверяем открытые UI панели - если открыты, не управляем
            if (anyPanelOpen)
            {
                return;
            }
            ...
            bool wantsToCrouch = canCrouch && Keyboard.current != null && Keyboard.current.leftCtrlKey.isPressed && !isSliding;
            ...
            if (canSlide && isSprinting && Keyboard.current != null && Keyboard.current.leftCtrlKey.wasPressedThisFrame && isGrounded)
            {
                isSliding = true;
        ...
        private void HandleMovement()
        {
            ...
            isSprinting = canSprint && Keyboard.current != null && Keyboard.current.leftShiftKey.isPressed && moveInput.y > 0.1f && isGrounded && !isCrouching && !isSliding;
            ...
            {
                if (canJump && Keyboard.current != null && Keyboard.current.spaceKey.wasPressedThisFrame && !isSliding)
                {
